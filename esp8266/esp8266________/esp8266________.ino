...
unsigned int TCP_SERVER_PORT=8344; //服务器端口//TCP创客云端口8344//TCP设备云端口8340
  ...
  {//data ready
    ...
    //////字符串匹配，检测发了的字符串TcpClient_Buff里面是否包含&msg=off，如果有，则关闭开关
      ...
      turnOffLed();
        ...
        if((TcpClient_Buff.indexOf("&msg1=on") > 0)) {
      turnOnLed1();
    //////字符串匹配，检测发了的字符串TcpClient_Buff里面是否包含&msg=off，如果有，则关闭开关
    }else if((TcpClient_Buff.indexOf("&msg1=off") > 0)) {
      turnOffLed1();
    }
