...
String UID = "d9855e89bd49313921fa42387cbdac52";
//主题名字，可在控制台新建
 ...
 //检查是否断开，断开后重连
  ...
  else
  {
    ...
    if(millis() - preHeartTick >= upDataTime){//上传数据
      ...
      */
      tusi=analogRead(tusipin);//读取A0口的电压值并赋值到val
      String upstr = "";
      upstr = "cmd=2&uid="+UID+"&topic="+TOPIC+"&msg=#"+temperature+"#"+humidity+"#"+my_led_status+"#"+tusi+"#\r\n";
  ...
  {//data ready
    ...
    if((TcpClient_Buff.indexOf("&msg=1on") > 0)) {
      turnOnLed1();
    //////字符串匹配，检测发了的字符串TcpClient_Buff里面是否包含&msg=off，如果有，则关闭开关
    }else if((TcpClient_Buff.indexOf("&msg=1off") > 0)) {
      turnOffLed1();
    }
